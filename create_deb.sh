...
find "${OPT_DIR}/lib/" -type f -name "*.so*" | xargs ldd | grep "=> /" | grep -v "/lib/x86_64-linux-gnu/libc.so" | grep -v "/lib/x86_64-linux-gnu/libm.so" | grep -v "/lib/x86_64-linux-gnu/libpthread.so" | awk '{print $3}' | xargs -I {} cp -u -v {} "${OPT_DIR}/lib/" 2>/dev/null
